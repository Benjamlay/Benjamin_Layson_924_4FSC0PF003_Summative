...
void GameSpriteManager::AddSpriteAtRandomPosition(TextureManager& texture_manager) {
    AddSprite({
...
void GameSpriteManager::AddSpriteAtMouse(const sf::Vector2f mousePos, TextureManager& texture_manager) {
    AddSprite(mousePos, texture_manager);
}

void GameSpriteManager::AddSprite(sf::Vector2f position, TextureManager& texture_manager) {
    sprites_.push_back(std::make_unique<GameSprite>(position, texture_manager));
}

void GameSpriteManager::Update(const float deltaTime, TextureManager& texture_manager) {
    // Mise à jour de tous les sprites
    for (const auto& sprite : sprites_) {
        sprite->Update(deltaTime, texture_manager);
    }
