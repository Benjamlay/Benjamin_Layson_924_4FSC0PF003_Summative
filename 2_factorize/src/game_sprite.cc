...
GameSprite::GameSprite(const sf::Vector2f pos, TextureManager& texture_manager)
    : position_(pos), alpha_(255.0f), is_active_(true) {

    gen_ = std::mt19937(rd_());
    color_dist_ = std::uniform_int_distribution<>(0, 255);
    texture_idx_dist_ = std::uniform_int_distribution<>(0, static_cast<int>(texture_manager.GetTextureCount()));
...
void GameSprite::Update(const float deltaTime, TextureManager& texture_manager) {

    texture_ = texture_manager.GetTexture(texture_idx_);
